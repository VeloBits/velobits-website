    ...
    <>
      ...
      >
        ...
        >
          ...
          <button
            ref={toggleRef}
            className="flex min-[641px]:hidden h-11 w-11 shrink-0 cursor-pointer items-center justify-center rounded-[8px] border-none bg-transparent p-[0.4rem] text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
        ...
        {menuOpen && (
          ...
          >
            ...
            <div className="mt-1 border-t border-border-subtle pt-3">
              <Link
                href="/#waitlist"
                className="btn btn-primary flex w-full max-w-full justify-center text-[0.9rem]"
                onClick={() => setMenuOpen(false)}
              >
                Join Waitlist →
              </Link>
      ...
      {/* Mobile menu backdrop — sits below the header (z-100) but above page content */}
      {menuOpen && (
        <div
          className="fixed inset-0 z-[95] bg-[rgba(0,0,0,0.45)] min-[641px]:hidden"
          aria-hidden="true"
          onClick={() => setMenuOpen(false)}
        />
      )}
